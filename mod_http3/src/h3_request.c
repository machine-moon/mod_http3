...
/* ap_find_linked_module walks every loaded module comparing names, and this was
 * called once per connection and again for every request. The answer cannot
 * change after startup, so resolve it once. */
...
static int peer_addr_resolve(h3q_engine* engine, h3q_conn* qconn, apr_pool_t* pool, apr_sockaddr_t** addr, char** client_ip)
{
    ...
    /* Fill locals first: apr_sockaddr_info_get NULLs its out-param on failure. */
